...
namespace KSPTraceNet20
{
    public class TraceHttpModule : IHttpModule
    {
        ...
        public void Dispose()
        {
            try
            {
                QueueManager.getInstance().emptyQueue();
            }catch(Exception ex)
            {
                log.Error(ex);
            }
        ...
        public void Init(HttpApplication context)
        {
            ...
            try
            {
                ...
                string KSPTrace_LoadLog4Net = ConfigurationManager.AppSettings["KSPTrace_LoadLog4Net"];
                if (!string.IsNullOrEmpty(KSPTrace_LoadLog4Net) && KSPTrace_LoadLog4Net.ToUpper().Equals("TRUE"))
                {
                    log4net.Config.XmlConfigurator.Configure();
                    log.Debug("TraceHttpModule.Init()");
                }
        ...
        private void KSPTrace(HttpApplication ha)
        {
            ...
            TraceData td = new TraceData { ContentType = contentType, UserAgent = agent, RequestUrl = url, RequestData = formData, UserName = username, AppId = KSPTrace_AppId, HttpMethod = httpMethod, FileNames = fileNames };
